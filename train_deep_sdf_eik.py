...
def main_function(experiment_directory, continue_from, batch_split):
    ...
    logging.info("torch num_threads: {}".format(torch.get_num_threads()))
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for sdf_data, indices in sdf_loader:
            ...
            sdf_data.requires_grad = True  # False
            ...
            for i in range(batch_split):
                ...
                # sdf grad for eik
                pred_sdf_grad = gradient(inputs=input, outputs=pred_sdf)
                ...
                # eikonal regularization, TODO: compute per-sample loss before mean?
                eik_loss = torch.abs(pred_sdf_grad.norm(2, dim=-1) - 1)
                eik_loss[sdf_gt[i].squeeze() < 0.1] = 0.
                chunk_loss += 0.25 * eik_loss.mean()
