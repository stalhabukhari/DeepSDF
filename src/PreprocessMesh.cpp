...
template <typename T>
pangolin::Image<T> GetImageWrapper(std::vector<T> &vec,
                                   size_t count_per_element) {
    PANGO_ASSERT(vec.size() % count_per_element == 0);
    if (vec.size()) {
        return pangolin::Image<T>(vec.data(), count_per_element,
...
pangolin::Geometry LoadGeometryObj(const std::string &filename) {
    ...
    if (tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err,
                         filename.c_str(),
                         pangolin::PathParent(filename).c_str())) {
        ...
        for (size_t i = 0; i < materials.size(); ++i) {
            if (!materials[i].diffuse_texname.empty()) {
                const std::string tex_name =
                    pangolin::FormatString("texture_%", i);
                try {
                    pangolin::TypedImage &tex_image = geom.textures[tex_name];
                    tex_image =
                        pangolin::LoadImage(pangolin::PathParent(filename) +
                    ...
                    for (std::size_t y = 0; y < (tex_image.h >> 1); ++y) {
                        std::memcpy(tmp_row.data(), tex_image.RowPtr(y),
                                    row_bytes);
                        std::memcpy(tex_image.RowPtr(y),
                                    tex_image.RowPtr(tex_image.h - 1 - y),
                                    row_bytes);
                        std::memcpy(tex_image.RowPtr(tex_image.h - 1 - y),
                                    tmp_row.data(), row_bytes);
                    }
        ...
        //        PANGO_ASSERT(all_of(
        //            [&](const size_t& v){return (v == 0) || (v ==
        //            num_verts);}, attrib.normals.size() / 3,
        //            attrib.colors.size() / 3));

        // Get rid of color buffer if all elements are equal.
        if (std::adjacent_find(attrib.colors.begin(), attrib.colors.end(),
                               std::not_equal_to<float>()) ==
            attrib.colors.end()) {
            attrib.colors.clear();
        }

        pangolin::Image<float> tiny_vs = GetImageWrapper(attrib.vertices, 3);
        pangolin::Image<float> tiny_ns = GetImageWrapper(attrib.normals, 3);
        pangolin::Image<float> tiny_cs = GetImageWrapper(attrib.colors, 3);
        pangolin::Image<float> tiny_ts = GetImageWrapper(attrib.texcoords, 2);

        // Some vertices are used with multiple texture coordinates or multiple
        // normals, and will need to be split. First, we'll find the number of
        // unique combinations of vertex, texture, and normal indices used, as
        // each will result in a separate vertex in the buffers
        ...
        for (auto &shape : shapes) {
            ...
            for (size_t i = 0; i < num_indices; ++i) {
                ...
                if (reindex_map.find(index) == reindex_map.end()) {
                    ...
                    reindex_map.insert(std::pair<tinyobj::index_t, std::size_t>(
                        index, reindex_map.size()));
                }
            }
        }
        ...
        pangolin::Image<float> new_vs, new_ns, new_cs, new_ts;
        {
            verts.Reinitialise(
                sizeof(float) * (tiny_vs.w + tiny_ns.w + tiny_cs.w + tiny_ts.w),
                num_unique_verts);
            size_t float_offset = 0;
            if (tiny_vs.IsValid()) {
                new_vs = verts.UnsafeReinterpret<float>().SubImage(
                    float_offset, 0, 3, num_unique_verts);
                verts.attributes["vertex"] = new_vs;
                float_offset += 3;
                for (auto &el : reindex_map) {
                    new_vs.Row(el.second).CopyFrom(
                        tiny_vs.Row(el.first.vertex_index));
                }
            }
            if (tiny_ns.IsValid()) {
                new_ns = verts.UnsafeReinterpret<float>().SubImage(
                    float_offset, 0, 3, num_unique_verts);
                verts.attributes["normal"] = new_ns;
                float_offset += 3;
                for (auto &el : reindex_map) {
                    new_ns.Row(el.second).CopyFrom(
                        tiny_ns.Row(el.first.normal_index));
                }
            }
            if (tiny_cs.IsValid()) {
                new_cs = verts.UnsafeReinterpret<float>().SubImage(
                    float_offset, 0, 3, num_unique_verts);
                verts.attributes["color"] = new_cs;
                float_offset += 3;
                for (auto &el : reindex_map) {
                    new_cs.Row(el.second).CopyFrom(
                        tiny_cs.Row(el.first.vertex_index));
                }
            }
            if (tiny_ts.IsValid()) {
                new_ts = verts.UnsafeReinterpret<float>().SubImage(
                    float_offset, 0, 2, num_unique_verts);
                verts.attributes["uv"] = new_ts;
                float_offset += 2;
                for (auto &el : reindex_map) {
                    new_ts.Row(el.second).CopyFrom(
                        tiny_ts.Row(el.first.texcoord_index));
                }
            }
        ...
        for (auto &shape : shapes) {
            ...
            auto faces =
                geom.objects.emplace(shape.name, pangolin::Geometry::Element());

            if (std::all_of(shape.mesh.num_face_vertices.begin(),
                ...
                pangolin::Image<uint32_t> new_ibo =
                    faces->second.UnsafeReinterpret<uint32_t>().SubImage(
                        0, 0, 3, num_faces);
                for (size_t f = 0; f < num_faces; ++f) {
                    for (size_t v = 0; v < 3; ++v) {
                        new_ibo(v, f) =
                            reindex_map[shape.mesh.indices[3 * f + v]];
                    }
                }
                ...
                throw std::runtime_error(
                    "Do not support meshes with mixed triangles and quads.");
            }
        }
    } else {
        throw std::runtime_error(pangolin::FormatString(
            "Unable to load OBJ file '%'. Error: '%'", filename, err));
    }
...
ToGlGeometryElement(const pangolin::Geometry::Element &el,
                    pangolin::GlBufferType buffertype) {
    pangolin::GlGeometry::Element glel(buffertype, el.SizeBytes(),
                                       GL_STATIC_DRAW, el.ptr);
    for (const auto &attrib_variant : el.attributes) {
        visit(
            [&](auto &&attrib) {
                ...
                glattrib.gltype =
                    pangolin::GlFormatTraits<typename T::PixelType>::gltype;
...
pangolin::GlGeometry ToGlGeometry(pangolin::Geometry &geom) {
    pangolin::GlGeometry gl;
    for (const auto &b : geom.buffers)
        gl.buffers[b.first] =
            ToGlGeometryElement(b.second, pangolin::GlArrayBuffer);

    for (const auto &b : geom.objects)
        gl.objects.emplace(
            b.first,
            ToGlGeometryElement(b.second, pangolin::GlElementArrayBuffer));
...
int main(int argc, char **argv) {
    ...
    pangolin::Geometry geom = LoadGeometryObj(meshFileName);
    ...
    pangolin::GlGeometry gl_geom = ToGlGeometry(geom);
