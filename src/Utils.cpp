...
float BoundingCubeNormalizationFromTinyObj(std::vector<tinyobj::shape_t> &geom,
    ...
    for (const auto &object : geom) {
        auto vertIndices = object.mesh.indices;
        for (const tinyobj::index_t &idx : vertIndices) {
            verticesUsed[idx.vertex_index] = 1;
        }
    }
    /////////
    ...
    for (size_t i = 0; i < numVertices; i++) {
        ...
        if (verticesUsed[i] == 0)
            continue;
        xMin = fmin(xMin, attrib.vertices[3 * i] + 0);
        yMin = fmin(yMin, attrib.vertices[3 * i] + 1);
        zMin = fmin(zMin, attrib.vertices[3 * i] + 2);
        xMax = fmax(xMax, attrib.vertices[3 * i] + 0);
        yMax = fmax(yMax, attrib.vertices[3 * i] + 1);
        zMax = fmax(zMax, attrib.vertices[3 * i] + 2);
    }
    ...
    for (size_t i = 0; i < numVertices; i++) {
        ...
        if (verticesUsed[i] == 0)
            continue;
        attrib.vertices[3 * i + 0] -= xCenter;
        attrib.vertices[3 * i + 1] -= xCenter;
        attrib.vertices[3 * i + 2] -= xCenter;

        coords << attrib.vertices[3 * i + 0], attrib.vertices[3 * i + 1],
            attrib.vertices[3 * i + 2];

        const float dist = coords.norm();
        maxDistance = std::max(maxDistance, dist);
    }
    ...
    if (fitToUnitSphere) {
        for (size_t i = 0; i < numVertices; i++) {
            attrib.vertices[3 * i + 0] /= maxDistance;
            attrib.vertices[3 * i + 1] /= maxDistance;
            attrib.vertices[3 * i + 2] /= maxDistance;
        }
        maxDistance = 1;
    }
